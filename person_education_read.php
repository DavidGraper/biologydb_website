<?php
  ...
  while($row = $result->fetch(PDO::FETCH_ASSOC)) {
    ...
    if (isset($_GET['id']) and $row['id'] == $_GET['id']) {
      ...
      // Grad year
      echo '<td><input type="text" class="form-control" name="gradyear" value="' . $row['gradyear'] . '"></td>';
...
//      echo '<input type="hidden" name="personid" value="' . $row['personid'] . '">';
      ...
      echo '<td><a class="btn btn-primary" href="person_education.php?personid=' . $row['personid'] . '&id=' . $row['id'] . '" role="button">Update</a></td>';
      echo '<td><a class="btn btn-danger" href="person_education_delete.php?personid=' . $row['personid'] . '&id=' . $row['id'] . '" role="button">Delete</a></td>';
    }
    echo "</tr>";
  }
?>
