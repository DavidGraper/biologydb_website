<?php
  ...
  $sql = "select * from data_person_jobtitles where personid=" . $personid . " order by id";
  ...
  while($row = $result->fetch(PDO::FETCH_ASSOC)) {
    ...
    if (isset($_GET['id']) and $row['id'] == $_GET['id']) {
      ...
      // Biology Job Title
      echo "<td>";
      $currentjobtitleid = $row['jobtitleid'];
      ...
      echo '<td><a class="btn btn-danger" href="person_jobtitle.php?personid=' . $personid . '" role="button">Cancel</a></td>';
      echo '</form>';
    } else {
      echo "<td>" . $jobtitles[$row['jobtitleid']] . "</td>";
