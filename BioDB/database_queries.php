...
function getcommitteeinfo() {
	...
	$sql = "select id, committeecategory, committeename "
		. "from data_departmentcommittees "
		. "order by committeecategory, committeename";
	...
	while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
		...
		$object->committeecategory = $row['committeecategory'];
...
function getcommitteemembers($committeeid) {
	...
	$sql = "select a.id, b.lastname, b.firstname, a.chair from data_person_committeeassignments a " .
		"join data_persons b on a.personid = b.id " .
		"where a.committeeassignmentid = " . $committeeid . " " .
...
function getpeople($peopletypeid) {
	...
	$sql = "select id, lastname, firstname from data_persons "
		. "where persontypeid = " . $peopletypeid . " "
